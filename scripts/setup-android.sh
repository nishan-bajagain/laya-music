...
for pkg in "${PACKAGES[@]}"; do
    if "${SDKMANAGER}" --list_installed 2>/dev/null | grep -qF "${pkg}"; then
        ...
        # android-37 lives on the preview channel (channel=3); pass it for all
        # packages — it is harmless for packages that exist on stable too.
        yes | "${SDKMANAGER}" --channel=3 "${pkg}" 2>&1 | grep -v "^$" || true
        echo "==>   Done: ${pkg}"
    fi
done
