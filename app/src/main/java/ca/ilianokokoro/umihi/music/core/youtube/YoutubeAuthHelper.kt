...
object YoutubeAuthHelper {
    ...
    private fun generateSapisidHash(sapisidCookie: String, origin: String): String {
        ...
        // CRITICAL FIX: The SAPISID cookie value is URL-encoded (e.g. "abc%3Ddef").
        // YouTube's SAPISIDHASH algorithm requires the *decoded* value when computing
        // the SHA-1 hash. Hashing the raw encoded value produces a wrong Authorization
        // header, causing YouTube to reject the request with 401/403 — which is why
        // playback history was never reported to YouTube Music.
        val decodedCookie = try {
            URLDecoder.decode(sapisidCookie, "UTF-8")
        } catch (_: Exception) {
            sapisidCookie // fall back to raw value if decoding fails
        }
        val sapisidHash = sha1("$currentTime $decodedCookie $origin")
