...
class SearchViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun search() {
        // Explicit search (keyboard action) runs immediately.
        activeSearchJob?.cancel()
        activeSearchJob = viewModelScope.launch {
            performSearch(uiState.value.search)
        }
    }
    ...
    fun onSearchFieldChange(newValue: String) {
        _uiState.update {
            it.copy(
                search = newValue,
                screenState = if (newValue.isBlank()) {
                    ScreenState.Idle
                } else {
                    ScreenState.Typing
                }
        ...
        // Search-as-you-type without flooding the API. Cancelling the previous
        // job also prevents an older, slower response replacing newer results.
        activeSearchJob?.cancel()
        if (newValue.isBlank()) return

        activeSearchJob = viewModelScope.launch {
            delay(Constants.Search.DEBOUNCE_MS)
    ...
    private suspend fun performSearch(query: String) {
        ...
        // Set this before collection starts so there is never a blank frame
        // between the debounce finishing and the repository's Loading emission.
        _uiState.update { it.copy(screenState = ScreenState.Loading) }

        songRepository.search(normalizedQuery).collect { apiResult ->
            _uiState.update {
                it.copy(
