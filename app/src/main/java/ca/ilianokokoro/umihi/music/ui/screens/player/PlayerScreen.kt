...
fun PlayerScreen(
    ...
    val downloadedSong by remember(currentSong?.youtubeId) {
        currentSong?.youtubeId?.let { db.songRepository().observeDownloadedSong(it) }
            ?: flowOf(null)
    ...
    // LyricsScreen owns the queue sheet while it is presented. Keeping this
    // host active underneath the lyrics modal creates two ModalBottomSheets
    // subscribed to the same visibility flag and duplicates the queue list.
    if (uiState.isQueueModalShown && !showLyrics) {
        QueueBottomSheet(
