...
class DownloadRepository(appContext: Context) {
    ...
    suspend fun downloadSongStandalone(song: Song, useMetered: Boolean = false) {
        ...
        // Duplicate detection: check if this song is already being downloaded
        // as part of a playlist. This prevents the same song from downloading
        // twice simultaneously (wasting bandwidth and storage).
        val playlistWorkId = "${song.youtubeId}"  // playlist downloads use playlistId+songId
        val allWork = getExistingJobs(playlistWorkId)
        // Also check if the song is already downloaded
        val existingSong = localSongRepository.getSong(song.youtubeId)
        if (existingSong != null && existingSong.downloaded) {
            printd("Song ${song.title} is already downloaded — skipping standalone download")
            return
        }
