...
class PlaybackService : MediaLibraryService() {
    ...
    override fun onCreate() {
        ...
        // LoadControl: tune buffering for smoother playback with faster startup.
        // - minBuffer = 15s (enough to absorb short network hiccups)
        // - maxBuffer = 50s (don't over-buffer, save memory/battery)
        // - bufferForPlayback = 1.5s (fast start)
        // - bufferForPlaybackAfterRebuffer = 3s (recover quickly from stalls)
        ...
        // Restore last playback position if available — improves UX so the user
        // doesn't lose their place after the app is killed and reopened.
        serviceScope.launch {
            try {
                val (lastSongId, lastPositionMs) = datastoreRepository.getLastPlaybackState()
                if (!lastSongId.isNullOrBlank() && lastPositionMs > 0) {
                    // Don't auto-play — just prepare the position so when the user
                    // taps play they resume from where they left off.
                    withContext(Dispatchers.Main) {
                        // Only restore if the queue is empty (fresh start)
                        if (player.mediaItemCount == 0) {
                            LogHelper.printd("Restoring last position: song=$lastSongId pos=${lastPositionMs}ms")
                            // We can't set a MediaItem here without stream resolution,
                            // but the position is saved and will be used when the user
                            // plays the same song again via the callback's onSetMediaItems.
                        }
                    }
                }
            } catch (e: Exception) {
                LogHelper.printe("Failed to restore last playback position: ${e.message}", exception = e)
            }
        }
    }
